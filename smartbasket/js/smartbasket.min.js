					...
					calculateCaretPosition: function (t) {
						...
						if (t !== e) {
							...
							s > o
								? (s = 10 * e)
								: n >= s && n !== o
								? r.maskDigitPosMapOld[s] || ((n = s), (s = s - (p - c) - t), r.maskDigitPosMap[s] && (s = n))
								: s > n && (s = s + (c - p) + l);
						}
						return s;
					},
					...
					getMasked: function (t, a) {
						...
						for (; _(); ) {
							...
							y
								? (g.match(y.pattern)
										? (o[k](g),
										  y.recursive && (-1 === b ? (b = c) : c === v && c !== b && (c = b - m), v === b && (c -= m)),
										  (c += m))
										: g === n
										? (f--, (n = void 0))
										: y.optional
		...
		(t = jQuery),
			...
			(e = {
				...
				showProducts: function (s) {
					let r = t("<form>").attr({
						...
						l = t("<div>")
							.attr({ class: "smart-basket__product-quantity smart-basket__product-quantity_header" })
							.text("Кількість"),
						c = t("<span>").attr({ class: "smart-basket__info-icon" }).text("?"),
						p = t("<button>").attr({ class: "smart-basket__info-msgs-close" }).text("×"),
						d = t("<span>").attr({ class: "smart-basket__info-msgs" }).html("Ціна за 1 товар чи послугу").append(p),
						u = t("<div>")
					...
					let m = t("<div>")
							...
							.html(`Заг-на ціна / ${a.smartBasketCurrency}`),
					...
					for (let n in s) {
						...
						a.productQuantityWrapper ||
							(b.attr({ disabled: "disabled" }), f.attr({ disabled: "disabled" }), v.attr({ disabled: "disabled" })),
						...
						for (let a in s[n]) {
							...
							if (
								...
								"sbSize" === a &&
									void 0 !== s[n][a] &&
									(c.children("span").append(`<span class="smart-basket__product-size"> Розмір: ${s[n][a]}</span>`),
									d.val(s[n][a])),
					...
					if (a.productSize && a.productPrice) {
						...
						t("." + a.productElement)
							...
							e.click(function (e) {
								...
								console.log(o),
									s.attr({ "data-sb-product-price": i, "data-sb-product-size": n, "data-sb-id-or-vendor-code": o }),
									r.html(i);
							});
							...
							}
							{
								...
								(i.sbId = n),
									...
									a.productQuantityWrapper
										...
										: ((i.sbPriceCommon = +t(this).attr("data-sb-product-price") * +t(this).data("sbProductQuantity")),
										  i.sbPriceCommon.toFixed(2)),
									(r[n] = i),
									e.setProducts(r),
									l.empty(),
				...
				commonResult: function (e, s, r) {
					let n = 0,
						...
						o = t("<div>")
							...
							.text("Кошик пустий. Ви не додали жодного товару!")
							.fadeOut(),
						l = t("<div>")
					...
					if ((0 == +n ? o.fadeIn() : o.fadeOut(), r))
						...
						0 == +n && t("." + o.attr("class")).css("display", "block"),
							...
							t("." + d.attr("class")).html(`<span>Всього товарів: </span> ${n}`),
				...
				userForm: function () {
					...
					if ((c.append(p), a.agreement.isRequired)) {
						...
						(r = a.agreement.isLink
							...
							s.is(":checked") ? r.text("Оплатити замовлення") : r.text("Я не приймаю умов користувацької згоди"),
							e.click(function () {
								s.is(":checked")
									? (r.text("Я не приймаю умов користувацької згоди"),
									  s.val("Я не приймаю умов користувацької згоди"),
									  m.attr({ disabled: "disabled" }),
									  d.addClass("smart-basket__input-wrapper_error"))
									: (r.text("Оплатити замовлення"),
									  s.val("Оплатити замовлення"),
									  m.attr({ disabled: !1 }),
									  d.removeClass("smart-basket__input-wrapper_error"));
							}),
							d.append(s).append(e).append(r);
					}
					let u = t("<button>").attr({ class: "smart-basket__close-form" }).text("Продовжити закупи");
					e.closeModalBasket(u);
					let m = t("<button>")
							.attr({ class: "smart-basket__send-form", form: "smart-basket__form", type: "submit" })
			...
			(t.fn.smbasket = function (a) {
				return e[a]
					...
					: "object" != typeof a && a
					? void t.error("Метод " + a + " не знайдено")
